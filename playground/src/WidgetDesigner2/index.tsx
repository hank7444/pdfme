...
function DesignerApp() {
    ...
    if (designer.current) {
      ...
      const widget = {
        ...
        widgetSchema: {
          ...
          pageSize: widgetEditInfo.pageSizes[widgetEditInfo.pageCursor],
  ...
  const onChangeWidgetSelect = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (selectedWidget) {
      const { name, schemas, widgetSchema: { width, height, basePdf, position, pageCursor, pageSize } } = selectedWidget;
      const padding = getTemplatePadding(pageSize.width, pageSize.height, width, height, position);
      const template: Template = getBlankTemplate();
  ...
  const onRemoveBackgroundPDF = () => {
    if (designer.current) {
      const pdfFileName = document.getElementById('pdfFileName');
      pdfFileName!.textContent = '';

      const defaultTemplate = getBlankTemplate();
      const defaultBasePdf = defaultTemplate.basePdf;
      const template = cloneDeep(designer.current.getTemplate());

      const defaultWidgetEditInfo = getDefaultWidgetEditInfo();
      const { position, schemas } = widgetEditInfoRef.current;

      widgetEditInfoRef.current.pageCursor = 0;
      widgetEditInfoRef.current.pageSizes = defaultWidgetEditInfo.pageSizes;
      widgetEditInfoRef.current.pageSize = defaultWidgetEditInfo.pageSize;
      widgetEditInfoRef.current.basePdf = '';

      const padding = getTemplatePadding(
        defaultWidgetEditInfo.pageSize.width, 
        defaultWidgetEditInfo.pageSize.height,
        defaultWidgetEditInfo.width,
        defaultWidgetEditInfo.height, 
        position,
      );

      template.editWidgetInfo!.width = defaultWidgetEditInfo.width;
      template.editWidgetInfo!.height = defaultWidgetEditInfo.height;
      template.editWidgetInfo!.pageCursor = 0;
      template.editWidgetInfo!.padding = padding;
      template.basePdf = defaultBasePdf;
      ...
      content: (
        <>
          ...
          <button
            id="fileSelectButton"
            className="px-2 py-1 border rounded hover:bg-gray-100 active:bg-sky-700"
            onClick={onRemoveBackgroundPDF}
          >
            Remove background PDF
          </button>
        </>
      ),
    },
