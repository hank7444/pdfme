...
import { Widget } from './WidgetDesigner2/types';
import { createAddSchema } from './plugins/utils';


interface DraggableWidgetGroup {
  id: string;
  name: string;
  width: number;
  height: number;
...
function DesignerApp() {
  ...
  const widgetGroupsRef = useRef<DraggableWidgetGroup[]>([]);
  const [widgetGroups, setWidgetGroups] = useState<DraggableWidgetGroup[]>([]);

  useEffect(() => {
    try {
      const widgetGroups = localStorage.getItem("widgetGroups");
      let widgetGroupJSON: DraggableWidgetGroup[] = []; 

      const widgets = localStorage.getItem("widgets");
      let widgetJSON: Widget[] = [];

      if (widgets) {
        widgetJSON = JSON.parse(widgets);
      }

      if (widgetGroups) {
        widgetGroupJSON = JSON.parse(widgetGroups).map((wg) => {

          const defaultWidget = widgetJSON.find(widget => widget.widgetGroupId === wg.id);

          return {
            id: wg.id,
            name: wg.name,
            width: defaultWidget?.widgetSchema.width || 50,
            height: defaultWidget?.widgetSchema.height || 50,
            previewImage: defaultWidget?.previewImage || '',
          }
        })
      }
  ...
  useEffect(() => {
    ...
    const onDropEvent = (e: DragEvent) => {
      ...
      if (data) {
        ...
        console.log('@@@ widgetGroup: ', widgetGroup);
        ...
        if (designer.current && widgetGroup) {
          ...
          const newSchema = {
            name: '',
            type: 'widgetGroup',
            width: widgetGroup!.width, // default widget width
            height: widgetGroup!.height, // default widget height
            previewImage: widgetGroup!.previewImage || '', // default widget preview image
            position: {
